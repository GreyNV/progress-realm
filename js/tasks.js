...
function addStat(stat, amount, mult = 1) {
    const bonus = 1 + (state.prestige[stat] || 0) * 0.05;
    state.stats[stat] += amount * mult * state.time * bonus;
}
    ...
    readScrolls: new Task({
        ...
        effect(mult) {
            ...
            if (state.resources.scrolls >= scrollCost && state.resources.energy >= energyCost) {
                ...
                addStat('intelligence', 0.3, mult);
                addStat('wisdom', 0.1, mult);
                applyResourceCaps();
            }
        }
    }),
    ...
    arcaneExperiment: new Task({
        ...
        effect(mult) {
            if (state.resources.manaCores >= 2) {
                ...
                addStat('intelligence', 0.5, mult);
                addStat('wisdom', 0.5, mult);
                applyResourceCaps();
            }
        }
    }),
